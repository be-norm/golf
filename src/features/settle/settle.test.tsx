...
describe('SettleScreen — the burst', () => {
  ...
  it('does not fire when an old round is opened from the recent list', async () => {
    ...
    expect(await screen.findByText('Settle up')).toBeInTheDocument()
    // ANY coin, not the one it happens to use: the confetti falls as SPECKS so
    // it takes the 16-grid `coin-small` (the 32-grid one cannot render below
    // 32px at integer scale and would tower over the paper it falls with), but
    // a "did not fire" assertion naming one sprite passes for free the day
