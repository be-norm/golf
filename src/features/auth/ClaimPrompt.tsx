...
export function ClaimPrompt() {
  ...
  // Name the courses too. The prompt promises to move "local data" and the
  // library moves with it, so a sheet reading "0 rounds and 0 players" while
  // silently claiming a course list would be the opposite of opt-in (MAI-76).
  const parts = [
    `${r} round${r === 1 ? '' : 's'}`,
    `${p} player${p === 1 ? '' : 's'}`,
    `${c} course${c === 1 ? '' : 's'}`,
