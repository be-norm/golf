...
describe('ctp — carryovers', () => {
  ...
  /**
   * C11 — AN AWARD IS EVIDENCE THE HOLE WAS PLAYED, and it outranks a missing
   * score. These bets are decided on the tee: you tap the grid standing there,
   * before anybody writes a number down. So a par 3 with a recorded winner and
   * no score is a hole somebody hit a shot on and never scored — not one the
   * group never reached.
   *
   * Before this, the grid kept that cell LIT while the settlement ignored it
   * entirely: the whole pile died reporting "no par 3 left to win them" with a
   * named winner sitting on hole 16. The grid and the money have to agree.
   */
  it('C11: a par 3 awarded but never scored still pays, and banks the carry', () => {
    const round = carryRound()
    const log = new EventLog()
    // 1–15 scored; nobody ever posts a score on 16, but B is given it on the tee
    scoreHoles(round, log, Array.from({ length: 15 }, (_, i) => i + 1))
    award(log, 16, 'p-b')
    log.append({ type: 'round/completed' })
    const ctp = ctpOf(round, log)

    // the cell is lit, and the money agrees with it
    expect(ctp.awards!(16).filter((a) => a.taken).map((a) => a.playerId)).toEqual(['p-b'])
    expect(ctp.holeResults[3]).toEqual({ hole: 16, kind: 'won', winnerId: 'p-b', units: 4 })
    expect(ctp.settlement.perPlayerCents).toEqual({
      'p-a': -800,
      'p-b': 2400,
      'p-c': -800,
