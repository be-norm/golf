...
- Each tee has up to three per-hole rows, ALL aligned to the same hole columns:
  • yardages — the tee's yardage row. Use it as an alignment anchor: cards print per-nine totals (OUT for holes 1–9, IN for 10–18). After reading a tee's yardages, check they sum to its OUT and IN totals; if a nine doesn't add up, you mis-read or shifted that nine — re-read it. Because the yardage, Handicap, and par rows share the same columns, getting the yardages aligned pins the other two rows to the right holes.
